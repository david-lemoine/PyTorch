...
# Now that you had a glimpse of autograd, nn depends on autograd to define 
# models and differentiate them. An nn.Module contains layers, and a method 
...
# Let’s try a random 32x32 input. Note: expected input size of this net (LeNet) 
# is 32x32. To use this net on the MNIST dataset, please resize the images from 
# the dataset to 32x32.
...
# (2) nn.Module - Neural network module. Convenient way of encapsulating parameters, with 
# helpers for moving them to GPU, exporting, loading, etc.
# (3) nn.Parameter - A kind of Tensor, that is automatically registered as a parameter when
...
# There are several different loss functions under the nn package . A simple loss
# is: nn.MSELoss which computes the mean-squared error between the input and the target.
...
# To backpropagate the error all we have to do is to loss.backward(). You need 
# to clear the existing gradients though, else gradients will be accumulated to 
# existing gradients.
